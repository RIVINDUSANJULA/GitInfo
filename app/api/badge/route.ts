...
export async function GET(req: NextRequest) {
  ...
  const svg = `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="${width}" height="${height}" rx="${height/4}" fill="url(#grad)"/>
      <rect width="${width}" height="${height}" rx="${height/4}" fill="black" fill-opacity="0.1"/>
      
      <!-- Simplified Icon Background -->
      <rect x="${paddingX}" y="${(height - iconSize) / 2}" width="${iconSize}" height="${iconSize}" rx="4" fill="white" fill-opacity="0.2"/>
      ...
      <!-- Subtle highlight -->
      <rect x="1" y="1" width="${width - 2}" height="${height / 2}" rx="${height / 4}" fill="white" fill-opacity="0.1"/>
    </svg>
  `.trim();
